Настройка pass и browserpass на компе + password store на андроид
-----------------------------------------------------------------
...
`pass init <email>`
   >Почту нужно указывать, заданную во время создания ключей на предыдущем шаге. 
...
`pimport pass lastpass <lastpass_export.csv> --out ~/.password-store`\
    ...
    >`<lastpass_export.csv>` - экспортированный из ластпасс файл\
...
Автозаполнение в браузере реализует расширение browserpass. Настроек у расширения минимум,
имееются две горячие клавиши, их можно переназначить. К сожалению расширение не умеет
...
2. Шифруем экспортируемые ключи. Пароль из предыдущего шага.
